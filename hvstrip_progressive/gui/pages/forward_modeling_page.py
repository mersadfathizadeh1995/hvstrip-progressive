...
from PySide6.QtWidgets import (
    ...
    QSpinBox, QDoubleSpinBox, QCheckBox, QComboBox, QMessageBox, QTabWidget,
...
from .multi_profile_tab import MultiProfileTab
from ...core.soil_profile import SoilProfile, Layer, compute_halfspace_display_depth
...
class ForwardWorker(QThread):
    ...
    def __init__(self, model_path, config, engine_name="diffuse_field"):
        super().__init__()
    ...
    def run(self):
        try:
            freqs, amps = compute_hv_curve(
                self.model_path, self.config, engine_name=self.engine_name,
            )
...
class ForwardModelingPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        plot_options.addWidget(QLabel("HS %:"))
        self.hs_ratio_spin = QDoubleSpinBox()
        self.hs_ratio_spin.setRange(10, 100)
        self.hs_ratio_spin.setValue(25)
        self.hs_ratio_spin.setSingleStep(5)
        self.hs_ratio_spin.setSuffix("%")
        self.hs_ratio_spin.setToolTip("Half-space display depth as % of total finite depth")
        self.hs_ratio_spin.valueChanged.connect(self._update_plot)
        plot_options.addWidget(self.hs_ratio_spin)
    ...
    def _create_multi_tab(self):
        """Create the Multiple Profiles tab (delegated to MultiProfileTab)."""
        self._multi_tab = MultiProfileTab(
            get_freq_config=lambda: {
                "fmin": self.fmin_spin.value(),
                "fmax": self.fmax_spin.value(),
                "nf": self.nf_spin.value(),
            },
            get_output_dir=lambda: self.outdir_edit.text().strip(),
            get_engine_name=lambda: self.engine_combo.currentText(),
            parent=self,
        )
        self._multi_tab.status_message.connect(
            lambda msg: self.results_text.setText(msg)
        )
        return self._multi_tab
    ...
    def _build_engine_config(self) -> dict:
        """Build the config dict for the selected engine."""
        engine_name = self.engine_combo.currentText()
        engine_cfg = self._engine_settings_config.get(engine_name, {})
        base = {
            "fmin": self.fmin_spin.value(),
            "fmax": self.fmax_spin.value(),
            "nf": self.nf_spin.value(),
        ...
        if engine_name == "ellipticity":
            base["n_samples"] = base.pop("nf")
        base.update(engine_cfg)
        return base
    ...
    def _on_finished(self, result):
        ...
        # Compute Vs30 if profile available
        vs30_str = ""
        if self._active_profile is not None:
            try:
                from ...core.vs_average import vs_average_from_profile
                vs_res = vs_average_from_profile(self._active_profile)
                ext = " (extrap.)" if vs_res.extrapolated else ""
                vs30_str = f"\nVs30 = {vs_res.vs_avg:.1f} m/s{ext}"
            except Exception:
                pass
    ...
    def _draw_vs_profile(self, ax, profile: SoilProfile):
        ...
        hs_ratio = self.hs_ratio_spin.value() / 100.0
        for layer in profile.layers:
            vs_vals.append(layer.vs)
            if layer.is_halfspace:
                hs_ext = compute_halfspace_display_depth(
                    total_finite, hs_ratio=hs_ratio,
                )
                depths.append(depths[-1] + hs_ext)
            else:
