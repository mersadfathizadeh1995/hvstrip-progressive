...
def run_complete_workflow(initial_model_path: str, output_base_dir: str, 
    ...
    try:
        ...
        for i, step_folder in enumerate(step_folders, 1):
            ...
            if not model_files:
                print(f"    [X] No model file found in {step_folder}")
                continue
            ...
            try:
                ...
                print(f"    [X] Error computing HV curve: {e}")
                continue
        ...
        # =================================================================
        print(f"\n[3/3] Post-Processing & Visualization")
        print("-" * 40)
        ...
        for i, step_folder in enumerate(step_folders, 1):
            ...
            try:
                ...
                print(f"    [X] Error in post-processing: {e}")
                continue
        ...
        for step_name, step_data in results["step_results"].items():
            files = ["model_*.txt", "hv_curve.csv"]
            if 'hv_curve_png' in step_data:
                files.append("hv_curve.png")
            if 'vs_profile_png' in step_data:
                files.append("vs_profile.png")
            if 'summary_csv' in step_data:
                files.append("summary.csv")
            print(f"   [{step_name}]: {', '.join(files)}")
